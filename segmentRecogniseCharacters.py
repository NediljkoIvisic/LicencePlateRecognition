...
for i, character in enumerate(cropedCharacters):
    recognisedCharacter.append(predictFromModel(character,model,labels)) # Poziv funkcije za predvidanje
    
if recognisedCharacter[0]=="C" and recognisedCharacter[1]=="K":
    recognisedCharacter[0]=np.array(['Č'], dtype='<U1')
elif recognisedCharacter[0]=="K" and recognisedCharacter[1]=="Z":
    recognisedCharacter[1]=np.array(['Ž'], dtype='<U1')
elif recognisedCharacter[0]=="P" and recognisedCharacter[1]=="Z":
    recognisedCharacter[1]=np.array(['Ž'], dtype='<U1')
elif recognisedCharacter[0]=="S" and recognisedCharacter[1]=="I":
    recognisedCharacter[0]=np.array(['Š'], dtype='<U1')
elif recognisedCharacter[0]=="S" and recognisedCharacter[1]=="H":
    recognisedCharacter[0]=np.array(['Š'], dtype='<U1')
    recognisedCharacter[1]=np.array(['I'], dtype='<U1')
elif recognisedCharacter[0]=="V" and recognisedCharacter[1]=="Z":
    recognisedCharacter[1]=np.array(['Ž'], dtype='<U1')
elif recognisedCharacter[0]=="Z" and recognisedCharacter[1]=="U":
    recognisedCharacter[0]=np.array(['Ž'], dtype='<U1')
    
for i, character in enumerate(cropedCharacters):
    fig.add_subplot(grid[i])
    title = np.array2string(recognisedCharacter[i])
